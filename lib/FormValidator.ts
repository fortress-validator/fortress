import type { Locales, Plugin, Rules } from '@fortress-validator/types';
import FieldValidator from './FieldValidator';
import defaultLocales from './locales';
import defaultRules from './rules';
import type { FormValidatorArguments } from './types';
...
class FormValidator {
  ...
  public setLocale(locale: string): this {
    if (!(locale in this.locales)) {
      console.warn(`The "${locale}" locale is not registered.`);
    }
    this.locale = locale;
    return this;
  }
  ...
  public setFallbackLocale(locale: string): this {
    if (!(locale in this.locales)) {
      console.warn(`The "${locale}" fallback locale is not registered.`);
    }
